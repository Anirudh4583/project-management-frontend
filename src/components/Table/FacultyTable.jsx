...
function Row({ row, form, id, applicants, data }) {
  ...
  const handleAccept = (email) => {
    ...
    axios
      .post(
        `https://design-project-backend.herokuapp.com/api/user/accept/`,
        {
          formId: form,
          idea: row,
          email: email,
  ...
  const handleReject = (email) => {
    ...
    axios
      .post(
        `https://design-project-backend.herokuapp.com/api/user/reject/`,
        {
          formId: form,
          idea: row,
          email: email,
    ...
    <React.Fragment>
      <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
        ...
        <TableCell align="center">
          {applicants?.filter((item) => item.idea === row).length}
        </TableCell>
      </TableRow>
      <TableRow>
          ...
          <Collapse in={open} timeout="auto" unmountOnExit>
            {data.available[id] === '0' ? (
              <Box>
                ...
                >
                  {' '}
                  You have already accepted the Application of{' '}
                  <b>{AcceptedEmail}</b> for <b>{row}</b>.
                </Typography>
              </Box>
            ) : (
              <Box sx={{ margin: 1 }}>
...
function FacultyTable({ rows, form }) {
  ...
  const getData = () => {
    axios
      .get(
        `https://design-project-backend.herokuapp.com/api/user/applicants/${form}`,
        {
          headers: {
            accesstoken: getToken(),
          },
